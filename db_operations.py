...
# ===== Buyer Operations =====
def get_all_buyers(page=None, per_page=None, search_term=None):
    """
    Get all buyers from the database with optional pagination and search
    
    Args:
        page (int, optional): Page number (1-indexed)
        per_page (int, optional): Number of items per page
        search_term (str, optional): Search term to filter by name or contact person
        
    Returns:
        tuple: (buyers_list, total_count) where total_count is the total number of records
    """
    db = get_db_session()
    try:
        ...
        if search_term:
            ...
            query = query.filter(
                ...
                (Buyer.contact_person.ilike(search_term)) |
        ...
        if page is not None and per_page is not None:
            query = query.order_by(Buyer.name)
            query = query.offset((page - 1) * per_page).limit(per_page)
        
        buyers = query.all()
        return buyers, total_count
    finally:
        db.close()
...
def get_buyer_by_id(buyer_id):
    ...
    try:
        ...
        if hasattr(buyer_id, 'item'):  # Check if it's numpy type
...
def update_order_status(order_id, status):
    ...
    try:
        # Convert numpy.int64 to Python int if needed
        if hasattr(order_id, 'item'):  # Check if it's numpy type
...
def update_style_status(style_id, status):
    ...
    try:
        # Convert numpy.int64 to Python int if needed
        if hasattr(style_id, 'item'):  # Check if it's numpy type
...
def update_material_status(material_id, status, **kwargs):
    ...
    try:
        # Convert numpy.int64 to Python int if needed
        if hasattr(material_id, 'item'):  # Check if it's numpy type
        ...
        if material:
            ...
            for key, value in kwargs.items():
                if hasattr(material, key):
                    # Convert numpy types for kwargs values if needed
